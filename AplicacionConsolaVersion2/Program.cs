...
namespace AplicacionConsolaVersion2
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            switch (optionMenu)
            {
                ...
                case 3:
                    ...
                    {
                        Console.WriteLine("No tareas ni citas asiganadas");
                        Program.Main(args);
                    }
                ...
                case 4:
                    ...
                    if (indexBorrar >= 0 && Console.ReadLine().ToLower().Equals("s"))
                        taskList.RemoveAt(indexBorrar);


                    Console.WriteLine($"¡Tarea {indexBorrar} eliminada correctamente!");
        ...
        {
            if (taskList.Count == 0 && appointmentsList.Count == 0) Console.WriteLine("No había citas ni tareas para yer");
            ...
            foreach (Appointment x in appointmentsList.FindAll(x => x.fecha.Day == DateTime.Now.Day-1))
            {
                Console.WriteLine("******** La cita del día de ayer   *******");
                Console.WriteLine(x.info());
            }

            foreach (MyTask x in taskList.FindAll(x => x.fecha.Day == DateTime.Now.Day-1))
            {
